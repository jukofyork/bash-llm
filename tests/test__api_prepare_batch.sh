...
expect_success "Direct output to stdout" \
    './api_prepare_batch.sh "Tell me about bash" > "$test_dir/out1.jsonl" && verify_jsonl "$test_dir/out1.jsonl" 1'

expect_success "System and user messages" \
    './api_prepare_batch.sh "You are helpful" "Tell me about bash" > "$test_dir/out2.jsonl" && verify_jsonl "$test_dir/out2.jsonl" 1'

# Test file output options
expect_success "Output to file (-o)" \
    './api_prepare_batch.sh -o "$test_dir/out3.jsonl" "Tell me about bash" && verify_jsonl "$test_dir/out3.jsonl" 1'

expect_success "Append to file (-a)" \
    'touch "$test_dir/append.jsonl" && 
     ./api_prepare_batch.sh -a "$test_dir/append.jsonl" "Query 1" &&
     ./api_prepare_batch.sh -a "$test_dir/append.jsonl" "Query 2" &&
     verify_jsonl "$test_dir/append.jsonl" 2'

# Test file inputs
expect_success "System from file" \
    './api_prepare_batch.sh -s "$test_dir/system.txt" "Tell me about bash" > "$test_dir/out4.jsonl" && verify_jsonl "$test_dir/out4.jsonl" 1'

expect_success "User from file" \
    './api_prepare_batch.sh -u "$test_dir/user.txt" > "$test_dir/out5.jsonl" && verify_jsonl "$test_dir/out5.jsonl" 1'

expect_success "Both from files" \
    './api_prepare_batch.sh -s "$test_dir/system.txt" -u "$test_dir/user.txt" > "$test_dir/out6.jsonl" && verify_jsonl "$test_dir/out6.jsonl" 1'

# Test stdin
expect_success "User from stdin" \
    'echo "Hello from stdin" | ./api_prepare_batch.sh > "$test_dir/out7.jsonl" && verify_jsonl "$test_dir/out7.jsonl" 1'

expect_success "System from stdin" \
    'echo "Be helpful" | ./api_prepare_batch.sh -s - "Tell me about bash" > "$test_dir/out8.jsonl" && verify_jsonl "$test_dir/out8.jsonl" 1'

# Test custom ID consistency
expect_success "Custom ID consistency" \
    './api_prepare_batch.sh "Same message" > "$test_dir/id1.jsonl" &&
     ./api_prepare_batch.sh "Same message" > "$test_dir/id2.jsonl" &&
     diff <(jq -r .custom_id "$test_dir/id1.jsonl") <(jq -r .custom_id "$test_dir/id2.jsonl")'

# Test error conditions
expect_failure "No input provided" \
    './api_prepare_batch.sh' \
    "No user message provided"

expect_failure "Missing file" \
    './api_prepare_batch.sh -s nonexistent.txt "Hello"' \
    "No such file"

expect_failure "Too many arguments" \
    './api_prepare_batch.sh arg1 arg2 arg3' \
    "Unexpected argument"

expect_failure "Invalid option" \
    './api_prepare_batch.sh --invalid option' \
    "Unknown option"

# Test model override
expect_success "Model override" \
    './api_prepare_batch.sh -m gpt-4-turbo "Test message" > "$test_dir/model.jsonl" &&
